...
class FeedbackWidgetState extends State<FeedbackWidget>
  ...
  FeedbackController? _feedbackControllerForBinding;

  /// Context du [OverlayEntry] racine du feedback (descendant de [Overlay]) — requis pour
  /// [Overlay.of] car [Navigator] n’est pas ancêtre du [State] du [FeedbackWidget].
  BuildContext? _feedbackOverlayLayerContext;

  OverlayEntry? _fullScreenFormOverlay;
  int _deferredOpenFormAttempts = 0;
  ...
  void didChangeDependencies() {
    ...
    final c = FeedbackData.maybeOf(context)?.controller;
    final theme = FeedbackTheme.of(context);
    _feedbackControllerForBinding?.bindPresentFeedbackForm(null);
    _feedbackControllerForBinding = c;
    if (theme.useFullScreenFeedbackForm) {
      c?.bindPresentFeedbackForm(_openFullScreenForm);
    }
  }

  void _removeFullScreenFormOverlay() {
    _fullScreenFormOverlay?.remove();
    _fullScreenFormOverlay = null;
  }

  void _openFullScreenForm() {
    if (!mounted) return;
    final layerCtx = _feedbackOverlayLayerContext;
    if (layerCtx == null) {
      if (_deferredOpenFormAttempts > 12) {
        _deferredOpenFormAttempts = 0;
        return;
      }
      _deferredOpenFormAttempts++;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _openFullScreenForm();
      });
      return;
    }
    _deferredOpenFormAttempts = 0;
    if (_fullScreenFormOverlay != null) return;

    final st = context;
    final themeData = FeedbackTheme.of(layerCtx);
    final onFeedback = BetterFeedback.of(st).onFeedback;
    if (onFeedback == null) return;

    final overlayState = Overlay.of(layerCtx);

    _fullScreenFormOverlay = OverlayEntry(
      builder: (dialogContext) {
        final dialogTheme = ThemeData(
          brightness: themeData.brightness,
          cardColor: themeData.feedbackSheetColor,
          colorScheme: themeData.colorScheme,
        );
        return Material(
          color: themeData.feedbackSheetColor,
          child: Theme(
            data: dialogTheme,
            child: SafeArea(
              child: Stack(
                ...
                children: [
                  Padding(
                    ...
                    child: widget.feedbackBuilder(dialogContext, (
                      String feedback, {
                      Map<String, dynamic>? extras,
                    }) async {
                      _removeFullScreenFormOverlay();
                      await _sendFeedback(
                        st,
                        onFeedback,
    ...
    overlayState.insert(_fullScreenFormOverlay!);
  }
  ...
  void dispose() {
    _removeFullScreenFormOverlay();
    _feedbackControllerForBinding?.bindPresentFeedbackForm(null);
    _feedbackControllerForBinding = null;
  ...
  bool backButtonIntercept() {
    if (_fullScreenFormOverlay != null) {
      _removeFullScreenFormOverlay();
      return true;
    }
  ...
  Widget build(BuildContext context) {
    ...
    ).chain(CurveTween(curve: Curves.easeInSine)).animate(_controller);
      ...
      initialEntries: [
        OverlayEntry(
          builder: (layerContext) {
            _feedbackOverlayLayerContext = layerContext;
            return Theme(
              data: theme,
              child: Material(
                color: FeedbackTheme.of(layerContext).background,
                child: AnimatedBuilder(
                  ...
                  builder: (context, screenshotChild) {
                    final useFullScreenForm =
                        feedbackThemeData.useFullScreenFeedbackForm;
                    return CustomMultiChildLayout(
                      delegate: _FeedbackLayoutDelegate(
                        ...
                        layoutFeedbackSheet: !useFullScreenForm,
                      ),
                      children: [
                        LayoutId(
                          id: _screenshotId,
                            ...
                            child: Padding(
                              ...
                              child: ScaleAndFade(
                                ...
                                child: ControlsColumn(
                                  ...
                                  onExpandFeedbackSheet:
                                      feedbackThemeData
                                          .useFullScreenFeedbackForm
                                      ? () => FeedbackData.maybeOf(
                                          context,
                                        )?.controller.presentFeedbackForm()
                                      : feedbackThemeData.sheetIsDraggable
                                      ? () {
                                          final data = FeedbackData.maybeOf(
                                            context,
                                          );
                                          final sheet =
                                              data?.controller.sheetController;
                                          if (sheet == null ||
                                              !sheet.isAttached) {
                                            return;
                                          }
                                      ...
                                      : null,
                                  controlsColumnExtraActions: feedbackThemeData
                                      .controlsColumnExtraActions,
                                ),
                              ),
                            ),
                          ),
                        if (!animation.isDismissed && !useFullScreenForm)
                          LayoutId(
                            id: _sheetId,
                            child:
  ...
  static Future<void> sendFeedback(
    ...
    await Future.delayed(delay, () async {
      ...
      await onFeedbackSubmitted(
        UserFeedback(text: feedback, screenshot: screenshot, extra: extras),
      );
    });
  }
...
class _FeedbackLayoutDelegate extends MultiChildLayoutDelegate {
  _FeedbackLayoutDelegate({
    ...
    required this.layoutFeedbackSheet,
  });
  ...
  final bool layoutFeedbackSheet;
  ...
  double get screenshotFraction {
    if (!layoutFeedbackSheet) {
      return 1.0 - (safeAreaHeight / screenHeight);
    }
    return 1 - sheetFraction - (safeAreaHeight / screenHeight);
  }
  ...
  void performLayout(Size size) {
    ...
    final Size controlsSize = layoutChild(
      _controlsColumnId,
      BoxConstraints.loose(Size(size.width, screenshotHeight)),
    );
    ...
    if (layoutFeedbackSheet) {
      ...
      final double sheetHeight = layoutChild(
        ...
        BoxConstraints.loose(
          Size(size.width, size.height - query.viewInsets.bottom),
        ),
      ).height;
      positionChild(
        _sheetId,
  ...
  bool shouldRelayout(covariant _FeedbackLayoutDelegate oldDelegate) {
    return query != oldDelegate.query ||
        ...
        layoutFeedbackSheet != oldDelegate.layoutFeedbackSheet;
  }
}
